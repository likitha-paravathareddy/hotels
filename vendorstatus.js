...
for(i=0;i<vendor_data.length;i++)
{
    ...
    if(vendor_data[i].status=="Approved")
            ...
            <th><div onclick="reject(hotel_name)" style="background:red;padding:10px;border-radius:10px;cursor:pointer;display:inline">Reject</div></th>
            </tr>`)
            }
    ...
    $("#usertab7").append(`<tr>
           <th style="padding:15px">${hotel_name}</th>
           <th>${src}</th>
           <th><div onclick="accept(hotel_name)" style="background:green;padding:10px;margin-left:5px;margin-right:5px;border-radius:10px;cursor:pointer;display:inline">Accept</div><div onclick="reject(hotel_name)" style="background:red;padding:10px;border-radius:10px;cursor:pointer;display:inline">Reject</div></th>
           </tr>`)
...
function accept(hotel_name) {
    ...
    $.post(url, JSON.stringify({ hotel_name: hotel_name }), function (xhr, status, responseText) {
        alert(responseText.responseText)
    });
};

function reject(hotel_name) {

    // $.post('http://127.0.0.1:3007/hotels/update',{email:email},(data,status)=>{
    //     alert(responseText.responseText)
    // })
    $.ajax({ type: "post", url: "http://127.0.0.1:3007/vendors/updatesreg", contentType: "application/json", data: JSON.stringify({hotel_name:hotel_name}), xhrFields: { withCredentials: false, }, headers: {}, success: function (data) { console.log("Success"); console.log(data); }, error: function () { console.log("We are sorry but our servers are having an issue right now"); }, });
